...
def create_csv(orig_tsv_file, csv_file, data_folder, accented_letters=False, language="en"):
    """
    ...
    for line in tzip(loaded_csv):
        ...
        if language == "fr":
            ...
            ALEF_MADDA = "\u0622"
            ALEF_HAMZA_ABOVE = "\u0623"
            letters = "ابتةثجحخدذرزسشصضطظعغفقكلمنهويىءآأؤإئ" + HAMZA + ALEF_MADDA + ALEF_HAMZA_ABOVE
...
def check_commonvoice_folders(data_folder):
    ...
    if not os.path.exists(data_folder + files_str):
        ...
        err_msg = "the folder %s does not exist (it is expected in " "the Common Voice dataset)" % (
            data_folder + files_str
        )
